...
Improved version: uses ENRICHMENT (firing rate on label X / baseline rate
of label X) instead of raw purity, which fixes the bias toward whatever
label is most common overall. Also computes, for each feature in the
chosen seed, its average stability (cosine similarity) against all other
9 seeds, so we can directly test the hypothesis: do stable features show
stronger structural enrichment than unstable ones?

Usage:
    python analyze_feature_biology_v2.py
"""
...
csv_path = OUTPUT_DIR / f"feature_enrichment_stability_layer{LAYER}_seed{SEED}.csv"
